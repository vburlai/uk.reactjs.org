# uk.reactjs.org

Files that need translation (sorted by size):

 | size | file |
 | ---- | ---- |
 | 36K | implementation-notes.md _(low priority)_ |
 | 23K | accessibility.md |
 | 21K | optimizing-performance.md |
 | 19K | design-principles.md _(low priority)_ |
 | 17K | integrating-with-other-libraries.md |
 | 13K | addons-animation.md |
 | 13K | static-type-checking.md |
 | 12K | how-to-contribute.md _(low priority)_ |
 | 12K | codebase-overview.md _(low priority)_ |
 | 11K | render-props.md |
 | 8.1K | reconciliation.md |
 | 7.8K | strict-mode.md |
 | 6.8K | legacy-context.md |
 | 5.6K | addons-two-way-binding-helpers.md |
 | 5.4K | forwarding-refs.md |
 | 4.9K | addons-perf.md |
 | 4.6K | faq-versioning.md |
 | 4.6K | addons-update.md |
 | 3.3K | uncontrolled-components.md |
 | 3.2K | faq-structure.md |
 | 2.6K | web-components.md |
 | 2.5K | addons-create-fragment.md |
 | 2.5K | addons.md |
 | 2.3K | cross-origin-errors.md |
 | 2.3K | faq-ajax.md |
 | 1.9K | reference-pure-render-mixin.md |
 | 1.7K | addons-shallow-compare.md |
 | 1.7K | faq-internals.md |
 | 1.6K | addons-pure-render-mixin.md |
 | 668B | faq-build.md |
 | 540B | error-decoder.md |
